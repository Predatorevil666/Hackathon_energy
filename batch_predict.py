...
def load_data(input_path: str) -> pd.DataFrame:
    ...
    if df['consumption'].dtype == 'object' and isinstance(df['consumption'].iloc[0], str):
        import json
...
def main():
    ...
    try:
        ...
        if model is None:
            logger.warning("Модель не загружена, будет использована логика по умолчанию")
        else:
