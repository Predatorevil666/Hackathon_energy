...
def clean_address_dadata(address):
    ...
    if not DADATA_API_KEY or not DADATA_SECRET_KEY:
        logger.warning("DaData API ключи не найдены. Стандартизация адреса пропущена.")
        return {
            "is_available": False,
            ...
            organizations = [
                {
                    ...
                    "type": item.get("rubrics", [{}])[0].get("name") 
                        if item.get("rubrics") else None
                } 
                for item in result["result"]["items"] if "name" in item
            ]
            return {
                "is_available": True,
                "data": {
...
def check_address_in_fns(address):
    ...
    commercial_keywords = [
        ...
        'логистический[ -]центр', 'деловой[ -]центр', 'корпус'
    ]
...
def analyze_address_type(address, sources):
    ...
    dadata_info = sources["dadata"]["data"] if sources["dadata"]["is_available"] and sources["dadata"]["data"] else None
    if dadata_info:
        # Если есть квартира - вероятнее физическое лицо
        if dadata_info.get("flat"):
            is_commercial_prob -= 0.2
            evidence.append(f"Адрес содержит квартиру {dadata_info.get('flat')}")
    ...
    # Анализ данных 2ГИС
    twogis_info = sources["twogis"]["data"] if sources["twogis"]["is_available"] and sources["twogis"]["data"] else None
    ...
    # Анализ данных ФНС
    fns_info = sources["fns"]["data"] if sources["fns"]["is_available"] and sources["fns"]["data"] else None
    ...
    if re.search(r'\bофис\s*\d+\b|\bпомещение\s*\d+\b|\bэтаж\s*\d+\b|\bбизнес[-\s]центр\b|\bтц\b', address_lower):
        is_commercial_prob += 0.3
...
def analyze_address_with_mistral(address):
    ...
    if not MISTRAL_API_KEY:
        logger.warning("MISTRAL_API_KEY не найден. Анализ с помощью AI пропущен.")
        return {
            "is_available": False,
            "data": None
        }
            ...
            try:
                ...
                if json_match:
                    ...
                    if "is_commercial" in result_data:
                        ...
                        logger.warning(f"Неожиданный формат ответа API: {result_data}")
                else:
...
def check_address(address_data):
    ...
    if OPEN_SOURCES_AVAILABLE:
        logger.info("Проверка через открытые источники (ФИАС, ФНС, Росреестр)...")
    ...
    if OPEN_SOURCES_AVAILABLE and open_sources_result:
        ...
        if "fias" in open_sources_result["sources"]:
            sources["fias_open"] = {
                "name": "ФИАС (открытый)",
                "is_available": open_sources_result["sources"]["fias"]["is_available"],
        ...
        if "fns" in open_sources_result["sources"]:
            sources["fns_open"] = {
                ...
                "is_available": open_sources_result["sources"]["fns"]["is_available"],
        ...
        if "rosreestr" in open_sources_result["sources"]:
            sources["rosreestr"] = {
                "name": "Росреестр",
                "is_available": open_sources_result["sources"]["rosreestr"]["is_available"],
    ...
    elif OPEN_SOURCES_AVAILABLE and open_sources_result and open_sources_result.get("normalized_address"):
    ...
    if analysis_result:
        ...
        if OPEN_SOURCES_AVAILABLE and open_sources_result and open_sources_result.get("explanation"):
            explanation += f". Открытые источники: {open_sources_result['explanation']}"
            ...
            if open_sources_result.get("probability") is not None:
                ...
                probability = 0.6 * probability + 0.4 * open_sources_result["probability"]
        ...
        for key, source in sources.items():
            if source["is_available"]:
                confidence = None
                if key == "mistral" and source["data"] and "probability" in source["data"]:
    ...
    logger.info(f"Вероятность: {final_result.get('probability', 'Не указана')}")
    logger.info(f"Объяснение: {final_result.get('explanation', 'Нет объяснения')}")
...
if __name__ == "__main__":
    ...
    if result is not None:
        status = 'Физическое лицо' if result["is_physical"] else 'Юридическое лицо'
