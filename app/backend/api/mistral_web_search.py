...
def search_duckduckgo(query, num_results=3, retries=3, delay=2):
    ...
    for attempt in range(retries):
        try:
            ...
            if results:
                print(f"DuckDuckGo: успешно получено {len(results)} результатов")
                return results
        except Exception as e:
            print(f"DuckDuckGo поиск, попытка {attempt+1}: {e}")
            time.sleep(delay * (attempt + 1))  # Экспоненциальная задержка
    
    print("DuckDuckGo: не удалось получить результаты после нескольких попыток")
    return []
...
def search_google_serper(query, num_results=3):
    ...
    if not api_key:
        print("Serper API ключ не найден в переменных окружения (SERPER_API_KEY)")
        return []
    ...
    try:
        ...
        if 'query' in data and 'search' in data['query']:
            for item in data['query']['search']:
                title = item.get('title', '')
                snippet = re.sub(r'<[^>]+>', '', item.get('snippet', ''))  # Убираем HTML теги
...
"""
        ...
        try:
            ...
            error_msg = f"Ошибка при запросе к Mistral API для анализа: {str(e)}"
            print(error_msg)
...
if __name__ == "__main__":
    ...
    query = "Солнечные батареи для частного дома" if len(sys.argv) < 2 else sys.argv[1]
