...
class AddressRequest(BaseModel):
    ...
    include_details: bool = Field(
        True, description="Включить подробную информацию из всех источников"
    )
...
class Source(BaseModel):
    ...
    data: Optional[Dict[str, Any]] = Field(
        None, description="Данные от источника"
    )
    confidence: Optional[float] = Field(
        None, description="Уверенность источника (0-1)"
    )
...
class AddressResponse(BaseModel):
    ...
    is_physical: bool = Field(
        ..., description="Физическое лицо (true) или юридическое (false)"
    )
    is_commercial: bool = Field(
        ..., description="Юридическое лицо (true) или физическое (false)"
    )
    probability: float = Field(
        ..., description="Вероятность определения (0-1)", ge=0, le=1
    )
    explanation: Optional[str] = Field(
        None, description="Объяснение результата"
    )
    normalized_address: Optional[str] = Field(
        None, description="Нормализованный адрес"
    )
    sources: Optional[List[Source]] = Field(
        None, description="Информация об использованных источниках"
    )
...
async def check_address_endpoint(request: AddressRequest):
    ...
    try:
        ...
        if result is None:
            raise HTTPException(
                status_code=500, detail="Не удалось определить тип адреса"
            )

        # Логируем результат
        logger.info(
            f"Результат проверки адреса '{request.address}': {json.dumps(result, ensure_ascii=False)}"
        )
        ...
        logger.info(
            f"Формирование ответа: is_physical={is_physical}, is_commercial={is_commercial}, probability={probability}"
        )
        ...
        logger.info(
            f"Отправляем ответ: {json.dumps(response.dict(), ensure_ascii=False)}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка при проверке адреса: {str(e)}"
        )
...
async def check_address_get(
    ...
    include_details: bool = Query(
        False, description="Включить подробную информацию"
    ),
):
