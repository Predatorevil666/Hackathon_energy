    ...
    def search_address(address: str) -> Dict[str, Any]:
        ...
        try:
            ...
            street_match = re.search(
                r"\bул[. ]+([а-яА-Я-]+)", normalized_address
            )
            if street_match:
            ...
            office_match = re.search(
                r"\bоф(ис)?[. ]+(\d+[а-я]?)", normalized_address
            )
            ...
            if flat_match:
                ...
                parts["flat"] = (
                    office_match.group(2)
                    if office_match.group(2)
                    else office_match.group(1)
                )
            ...
            result = {
                ...
                "normalized_address": " ".join(
                    [
                        parts.get("city", ""),
                        f"ул. {parts.get('street', '')}"
                        if "street" in parts
                        else "",
                        f"д. {parts.get('house', '')}"
                        if "house" in parts
                        else "",
                        f"{parts.get('flat_type', 'кв.')} {parts.get('flat', '')}"
                        if "flat" in parts
                        else "",
                    ]
                ).strip(),
            ...
            return {"is_available": True, "data": result}
        ...
        try:
            ...
            result = {
                ...
                "explanation": (
                    ...
                    if found_keywords
                    else "Коммерческие признаки в адресе не найдены"
                ),
            }
...
class RosreestrIntegration:
    ...
    def check_property_info(address: str) -> Dict[str, Any]:
        ...
        try:
            logger.info(
                f"Поиск информации о недвижимости в Росреестре: {address}"
            )
            ...
            return {"is_available": True, "data": property_info}
...
def check_address_all_sources(address: str) -> Dict[str, Any]:
    ...
    # Голоса от ФИАС
    if (
        fias_result["is_available"]
        and fias_result["data"]
        and fias_result["data"].get("is_commercial") is not None
    ):
    ...
    # Голоса от ФНС
    if (
        fns_result["is_available"]
        and fns_result["data"]
        and fns_result["data"].get("is_commercial") is not None
    ):
    ...
    if (
        rosreestr_result["is_available"]
        and rosreestr_result["data"]
        and rosreestr_result["data"].get("is_commercial") is not None
    ):
        is_commercial_votes.append(rosreestr_result["data"]["is_commercial"])
        confidence_weights.append(
            rosreestr_result["data"].get("confidence", 0.5)
        )
    ...
    if fias_result["is_available"] and fias_result["data"]:
        if fias_result["data"].get("is_commercial"):
            explanation_parts.append(
                "ФИАС: адрес коммерческий (офисное помещение)"
            )
        elif fias_result["data"].get("is_commercial") is False:
            explanation_parts.append("ФИАС: адрес жилой (квартира)")

    if fns_result["is_available"] and fns_result["data"]:
        if fns_result["data"].get("found_keywords"):
            explanation_parts.append(
                f"ФНС: найдены признаки коммерческого помещения: {', '.join(fns_result['data']['found_keywords'])}"
            )
        elif fns_result["data"].get("organizations_count", 0) > 0:
            explanation_parts.append(
                f"ФНС: найдено {fns_result['data']['organizations_count']} организаций по адресу"
            )

    if rosreestr_result["is_available"] and rosreestr_result["data"]:
        if rosreestr_result["data"].get("property_type"):
            explanation_parts.append(
                f"Росреестр: тип помещения - {rosreestr_result['data']['property_type']}"
            )
...
if __name__ == "__main__":
    ...
    for address in test_addresses:
        ...
        print(
            f"Коммерческий: {'Да' if result.get('is_commercial') else 'Нет'}"
        )
