...
try:
    ...
    from mistral_web_search import web_search, analyze_with_mistral
except ImportError:
    try:
        from app.backend.api.mistral_api import query_mistral, setup_environment
        from app.backend.api.mistral_web_search import web_search, analyze_with_mistral
    except ImportError:
        raise ImportError("Не удалось импортировать модули mistral_api и mistral_web_search")
...
try:
    ...
    try:
        ...
        print("Модуль check_address_routes не найден, маршруты проверки адресов будут недоступны")
...
async def process_mistral_query(request: MistralRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка при запросе к Mistral API: {str(e)}")
...
async def process_web_search(request: WebSearchRequest):
    ...
    print(f"DEBUG: Получен запрос веб-поиска: query='{request.query}', num_results={request.num_results}, search_engines={request.search_engines}")
    
    # Проверяем корректность параметра search_engines
    if request.search_engines and (len(request.search_engines) == 1 and request.search_engines[0] == 'string'):
        print(f"DEBUG: Обнаружен некорректный параметр search_engines=['string'], устанавливаем значение None")
    ...
    try:
        ...
        try:
            ...
            print(f"DEBUG: Получены результаты поиска: {len(search_results)} результатов")
        except Exception as e:
        ...
        if not search_results:
            print(f"DEBUG: Поиск не дал результатов для запроса '{request.query}'")
        ...
        analysis = analyze_with_mistral(search_results, request.query, model=request.model)
        print(f"DEBUG: Анализ результатов выполнен, длина анализа: {len(analysis)} символов")
