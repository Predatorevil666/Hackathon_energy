...
def get_db_engine():
    ...
    logger.info(
        f"Подключение к базе данных: {db_url.replace(':'.join(db_url.split(':')[2:]).split('@')[0], '***:***')}"
    )
...
def load_osint_data(file_path):
    ...
    if file_ext == ".json":
        ...
        raise ValueError(
            f"Неподдерживаемый формат файла: {file_ext}. Поддерживаются .json и .csv"
        )
    ...
    try:
        with engine.connect() as conn:
            ...
            for i in range(0, len(records), batch_size):
                ...
                logger.info(
                    f"Обработано {i + len(batch)}/{len(records)} записей"
                )
...
def main():
    ...
    try:
        ...
        logger.info(
            f"Загрузка данных успешно завершена. Добавлено/обновлено {inserted} записей."
        )
        return 0
