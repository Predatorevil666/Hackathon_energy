...
from config.models_config import (
    ...
    MINIO_BUCKET, MODEL_KEY, TMP_MODEL_PATH,
    COMMERCIAL_THRESHOLD, FALLBACK_THRESHOLD
)
...
class ModelLoader:
    ...
    def load_model(self):
        ...
        try:
            ...
            logger.info(f"Downloading model from {self.bucket}/{self.model_key}")
...
def predict_records(data: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        if model is None:
            ...
            try:
                X = df[["total"]]  # В реальном проекте здесь будет больше признаков
...
def upload_model_to_s3(
    ...
    try:
        ...
        logger.info(f"Model successfully uploaded to s3://{bucket}/{model_key}")
        return True
    except Exception as e:
