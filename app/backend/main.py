...
try:
    from models import PredictionResponse, Record
except ImportError:
    try:
        from app.backend.models import PredictionResponse, Record
    except ImportError:
...
try:
    from config.api_config import API_DESCRIPTION, API_TITLE, API_VERSION
    from config.logging_config import logger

    from predict import predict_records
except ImportError:
    try:
        from app.backend.config.api_config import (
            API_DESCRIPTION,
            API_TITLE,
            API_VERSION,
        )
...
try:
    ...
    try:
        ...
        try:
            logger.warning(
                "Модуль Mistral API не найден, соответствующие маршруты будут недоступны"
            )
        except:
            print(
                "Модуль Mistral API не найден, соответствующие маршруты будут недоступны"
...
try:
    ...
    try:
        ...
        try:
            logger.warning(
                "Модуль проверки адресов не найден, соответствующие маршруты будут недоступны"
            )
        except:
            print(
                "Модуль проверки адресов не найден, соответствующие маршруты будут недоступны"
...
@app.post(
    "/predict", response_model=List[PredictionResponse], tags=["Prediction"]
)
async def predict(records: List[Record]):
    ...
    try:
        logger.info(
            f"Получен запрос на предсказание для {len(records)} объектов"
        )
        ...
        logger.info(
            f"Предсказания выполнены успешно: {len(predictions)} результатов"
        )
        return predictions
